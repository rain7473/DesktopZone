...
class ChartMarkerView(
    ...
    private val valueTitle: String = "Semana actual",
    private val previousValueTitle: String = "Semana pasada",
    private val labelFormatter: ((String) -> String)? = null
    ...
    override fun refreshContent(e: Entry?, highlight: Highlight?) {
        val idx = e?.x?.toInt() ?: 0
        val rawLabel = if (idx in labels.indices) labels[idx] else ""
        val label = labelFormatter?.invoke(rawLabel) ?: (diasCompletos[rawLabel] ?: rawLabel)

        tvLabel.text = label
        tvValue.text = "$valueTitle: $prefix%,.0f".format(e?.y ?: 0f)

        if (dataPasada.isNotEmpty() && idx in dataPasada.indices) {
            tvValuePasada.visibility = TextView.VISIBLE
            tvValuePasada.text = "$previousValueTitle: $prefix%,.0f".format(dataPasada[idx])
        } else {
